    ...
    <n-spin :show="loading" description="请稍候...">
      ...
      >
        <n-steps
          ...
          size="small"
        >
          <n-step description="仅支持上传.xlsx数据" title="上传数据" />
          <n-step description="修改数据基础信息" title="修改基础信息" />
          <n-step description="保存完数据后需要发布才能生效" title="发布" />
          <n-step description="数据创建完成" title="成功" />
        </n-steps>
        ...
        <n-form
          ...
          size="small"
        >
          <n-divider v-show="showStep" title-placement="left"> 基础信息</n-divider>
          ...
          <n-divider title-placement="left"> 字段确认</n-divider>

          <n-grid :cols="3" x-gap="12">
            <n-gi>
              <n-form-item label="品牌名字段" path="presetColumn.brandName">
                <n-select
            ...
            <n-gi>
              <n-form-item label="英文名字段" path="presetColumn.enName">
                <n-select
            ...
            <n-gi>
              <n-form-item label="品牌商字段" path="presetColumn.vendorName">
                <n-select
        ...
        <template #action>
          <n-space>
            ...
            <n-button
              v-if="params.id !== 0 && params.status === 0"
              :loading="formBtnLoading"
              type="info"
              @click="publish"
              >发布
...
<script lang="ts" setup>
  ...
  const filterFullColumns = computed(() => {
    ...
    fullColumns.value.forEach((item) => {
      ...
      if (
        item.value === params.value.presetColumn.brandName ||
